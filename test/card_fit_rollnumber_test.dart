...
void main() {
    ...
    test('every printable field has its own entry', () {
      for (final StudentField f in StudentField.printableRows) {
        expect(
          ...
          isTrue,
          reason: '${f.key} has no declared style, so it silently falls through '
  ...
  group('Roll number reaches the card', () {
    test('the row is laid out once it has a value', () async {
      final CardRenderPlan p =
          await plan('default_vertical.json', withRoll: true);
      expect(
    ...
    test('a blank roll number prints nothing', () async {
      final CardRenderPlan p =
          await plan('default_vertical.json', withRoll: false);
      expect(
        p.rows.map((PlannedRow r) => r.field),
  ...
  group('Measured fit per template', () {
    ...
    test('the two horizontal templates hold every field at full size',
        () async {
      for (final String name in <String>[
        ...
        final PlannedRow address = p.rows
            .firstWhere((PlannedRow r) => r.field == StudentField.address);
